...
def trim_detections(num_detections,
                    detection_scores,
                    detection_classes,
                    detection_boxes,
                    max_num_detections=10):
  """Trims the detections to the `max_num_detections`.
  ...
  Args:
    ...
    max_num_detections: Maximum number of detections.
...
class VBERT(ModelBase):
  """Wraps the BiLSTM layer to solve the VCR task."""

  def __init__(self, model_proto, is_training):
    super(VBERT, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.VBERT):
      raise ValueError('Options has to be an VBERT proto.')
  ...
  def predict(self, inputs, **kwargs):
    ...
    (image, image_height, image_width, num_detections, detection_boxes,
     detection_classes, detection_scores, answer_choices, answer_choices_len,
     answer_label) = (
    ...
    (num_detections, detection_scores, detection_classes,
     detection_boxes) = trim_detections(
         num_detections,
         detection_scores,
         detection_classes,
         detection_boxes,
